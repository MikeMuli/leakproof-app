...
export function renderEvent(event: NotificationEvent): { subject: string; body: string } {
  switch (event.type) {
    case "new_disputable_item":
      return {
        subject: `PayoutCheck: ${rm(event.gapCents)} flagged on ${event.shopName}`,
    ...
    case "weekly_digest":
      return {
        subject: `PayoutCheck weekly digest: ${event.periodLabel}`,
    ...
    case "payout_mismatch":
      return {
        subject: `PayoutCheck: payout mismatch on ${event.shopName}`,
    ...
    case "funds_on_hold":
      return {
        subject: `PayoutCheck: ${rm(event.amountCents)} newly on hold`,
