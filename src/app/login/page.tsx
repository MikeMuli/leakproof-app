  ...
  return (
    <div className="lp-auth">
      <div className="lp-auth-form">
        <span className="lp-mark lp-auth-form-mark"><i /> PayoutCheck</span>

        <h1 style={{ fontSize: 28, marginBottom: 6 }}>{mode === "signup" ? "Create your account" : "Welcome back"}</h1>
        <p className="sub" style={{ marginBottom: 24 }}>
          {mode === "signup" ? "Set a password to finish setting up your account." : "Sign in to see where your settlement money went."}
        </p>

        {mode === "signup" && !inviteCode && (
          <p role="alert" style={{ color: "var(--disputed)", fontSize: 13, marginBottom: 16 }}>
            PayoutCheck is invite-only right now. You need an invite link to create an account.
          </p>
        )}
        ...
        <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          {error && <p role="alert" style={{ color: "var(--disputed)", fontSize: 13 }}>{error}</p>}
          <button type="submit" disabled={busy || (mode === "signup" && !inviteCode)} className="lp-btn lp-btn-primary" style={{ padding: "12px 16px", marginTop: 4 }}>
        ...
        {mode === "signin" && (
          <p style={{ marginTop: 16, fontSize: 13, color: "var(--ink-3)" }}>
            New here? You&apos;ll need an invite link from your PayoutCheck contact.
          </p>
        )}
        {mode === "signup" && (
          <button onClick={() => setMode("signin")} className="lp-btn lp-btn-ghost" style={{ marginTop: 14, alignSelf: "start" }}>
